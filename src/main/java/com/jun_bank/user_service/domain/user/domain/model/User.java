...
public class User {
   ...
   * 이전 상태 (롤백용)
   * <p>
   * 상태 변경 전 이전 상태를 저장합니다.
   * Auth Server 호출 실패 등으로 롤백이 필요할 때 사용합니다.
   * </p>
   */
  ...
  public void withdraw() {
    this.previousStatus = this.status;  // 롤백용 저장
   ...
   * 사용자 탈퇴 처리 (삭제자 지정)
   * <p>
   * 상태를 DELETED로 변경하고 삭제자를 기록합니다.
   * </p>
   *
   * @param deletedBy 삭제자 ID (본인 또는 관리자)
   * @throws UserException 상태 전이가 허용되지 않는 경우 (USER_036)
   */
   ...
   * 탈퇴 취소 (롤백)
   * <p>
   * Auth Server 호출 실패 등으로 탈퇴 처리를 취소할 때 사용합니다.
   * 이전 상태로 복구하고 Soft Delete 필드를 초기화합니다.
   * </p>
   *
   * <h4>복구 내용:</h4>
   * <ul>
   *   <li>status → 이전 상태 (previousStatus)</li>
   *   <li>isDeleted → false</li>
   *   <li>deletedAt → null</li>
   *   <li>deletedBy → null</li>
   * </ul>
   *
   * @throws IllegalStateException 롤백할 이전 상태가 없는 경우
   */
  public void cancelWithdrawal() {
    if (this.previousStatus == null) {
      throw new IllegalStateException("롤백할 이전 상태가 없습니다.");
    }
   ...
   * <p>
   * 이전 상태로 복구합니다.
   * Auth Server 호출 실패 등 트랜잭션 실패 시 사용합니다.
   * </p>
   *
   * @throws IllegalStateException 롤백할 이전 상태가 없는 경우
   */
   ...
   * 이전 상태 초기화
   * <p>
   * 트랜잭션이 성공적으로 완료된 후 호출하여 롤백 데이터를 정리합니다.
   * </p>
   */
